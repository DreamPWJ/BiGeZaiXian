...
public class LogAdvice implements Ordered {
    ...
    @Pointcut("execution(* com.juhuawang..*.*(..))")
    //所有的方法调用
    ...
    public Object interceptor(ProceedingJoinPoint pjp) throws Throwable {
        ...
        if (logger.isDebugEnabled()&&flag) {
            logger.info("===比格在线程序运行方法点:  " + className + "." + methodName + "(" + argsvalue + ")" + " 方法进入开始！");
        }
        //proceed()就是调用目标方法 可控制目标方法是否执行
        Object object = pjp.proceed();
        //log4j.properties配置debug调试级别打印
        if (logger.isDebugEnabled()&&flag) {
            logger.info("===比格在线程序运行方法点:  " + className + "." + methodName + "(" + argsvalue + ")" + " 方法返回结束！");
        }
        return object;
    }

    //异常记录  异常增强处理
    @AfterThrowing(pointcut = "execution(* com.juhuawang..*.*(..))", throwing = "ex")
    public void interceptorException(Exception ex) throws Throwable {
        logger.info("=========  比格在线程序异常错误记录  =========");
        logger.info("===比格在线程序运行抛出的异常:    " + ex.getMessage());
        logger.info("===比格在线程序异常发生的原因： " + ex.getCause());
        logger.info("===比格在线程序运行异常详细信息：　　　" + ex.fillInStackTrace());
        logger.info("===比格在线程序异常发生时间:    " + new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date()));
    }
